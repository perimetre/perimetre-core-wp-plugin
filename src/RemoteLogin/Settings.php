...
namespace Perimetre\Core\RemoteLogin;

use Perimetre\Core\Admin\Tabs;

/**
 * Adds the "Remote Login" tab to the Settings > Perimetre Core admin page
 * (owned by Status\Settings).
 *
...
final class Settings
{
     ...
     * still submits under the `perimetre-core` option group (PAGE_SLUG).
     */
    public const SECTION_PAGE = 'perimetre-core-remote-login';

    private const PAGE_SLUG       = 'perimetre-core';
    ...
    public static function register_settings(): void
    {
        add_settings_section(
            self::SECTION_ID,
            __('Remote Login', 'perimetre-core'),
        ...
        add_settings_field(
            self::OPTION_ENABLED,
            __('Enable remote login', 'perimetre-core'),
        ...
        add_settings_field(
            ...
            __('Connection status', 'perimetre-core'),
    ...
    public static function render_api_key_field(): void
    {
        ...
        $placeholder = $stored === ''
            ? esc_attr__('Paste the API key shown by the portal', 'perimetre-core')
            : esc_attr__('(stored — leave blank to keep)', 'perimetre-core');
        printf(
    ...
    public static function render_status_field(): void
    {
        ...
        if ($connected_at !== '') {
            ...
            printf(
                ...
                esc_html__('Connected. Last handshake: %s', 'perimetre-core'),
                esc_html($connected_at)
            );
            echo '<p class="description">' .
                esc_html__('Saving will re-run the handshake automatically.', 'perimetre-core') .
                '</p>';
        } else {
            echo '<span style="color:#6b7280;font-size:14px;">○ </span>';
            echo esc_html__('Not yet connected. Click Save Changes to connect.', 'perimetre-core');
        }
    }
    ...
    public static function maybe_render_notices(): void
    {
        ...
        $messages = [
            'connected' => [
                ...
                'text'  => __('Connected to the Helm portal.', 'perimetre-core'),
            ],
            'failed'    => [
                'class' => 'notice-error',
                'text'  => __('Connection to the Helm portal failed. Check the portal URL and API key, then save again.', 'perimetre-core'),
            ],
            'missing'   => [
                'class' => 'notice-warning',
                'text'  => __('Remote login is enabled but the portal URL or API key is missing.', 'perimetre-core'),
            ],
        ];
