...
**`Preview\*`** — points WordPress's **Preview** button at a headless frontend. `Settings` owns the shared secret (options field on the Webhook settings page, overridable by the `PERIMETRE_PREVIEW_SECRET` wp-config constant). `PreviewUrl` filters `preview_post_link` (priority 20, so it wins over a project's own permalink rewrite) into `{frontend}/preview/{type}/{id}/?locale&exp&user&token`, HMAC-signed over `id|type|locale|exp|user` with a 12h expiry. `TokenAuth` accepts that same payload back as an `X-Preview-Auth` header on GraphQL requests and resolves `determine_current_user` to the editor baked into the token — so the frontend reads drafts with that editor's capabilities and no service account exists. `EditorPreviewPane` + `assets/editor-preview.js` add the side-by-side sidebar. **Two gates keep it inert:** no secret, or no `perimetre_core_preview_frontend_url` filter from Project Core, and the Preview button is untouched. Watch out for re-entrancy in `TokenAuth`: never call `user_can()`/`current_user_can()` inside `determine_current_user` — it recurses through `wp_get_current_user()` until PHP exhausts memory, which is why the capability check reads `WP_User::$allcaps` directly behind a `$resolving` guard.
